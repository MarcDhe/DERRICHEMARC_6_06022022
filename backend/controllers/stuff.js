...
const { find } = require('../models/Sauce'); // --> pourquoi  ?
...
exports.like = (req, res, next) => {
  ...
  Sauce.findOne({_id : req.params.id})
    .then(sauce => {
      console.log('sauce select :', sauce);
      ...
      if(req.body.like === 0 && sauce.userLiked.includes(req.body.userId)){
        Sauce.updateOne({_id : req.params.id}, {$inc : {likes : -1}, $pull: {userLiked : req.body.userId}})
          .then(() => res.status(200).json({ message : 'Like retiré !'}))
          .catch(error => res.status(400).json({ error }));  
      }
      if(req.body.like === -1 && !sauce.userDisliked.includes(req.body.userId) ){
        Sauce.updateOne({_id : req.params.id}, {$inc : {dislikes : 1}, $push: {userDisliked : req.body.userId}})
          .then(() => res.status(200).json({ message : 'Dislike ajouté !'}))
          .catch(error => res.status(400).json({ error }));   
      }
      if(req.body.like === 0 && sauce.userDisliked.includes(req.body.userId)){
